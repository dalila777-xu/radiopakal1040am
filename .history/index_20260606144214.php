  ...
  <style>
    ...
    * {
    ...
    header::before {
      ...
      opacity: 0.45; /* Corregido opacidad a valor CSS estándar */
      pointer-events: none;
    }
    ...
    th {
      background: #8B2C2D; /* Cambiado a armonía de color vino del sitio */
      color: white;
      padding: 14px 16px;
      text-align: left;
      font-weight: 600;
...
<body>
  ...
  <div class="container">
    ...
    <!-- MÓDULO 2: Programación (INTEGRADO CON LOCUTORES) -->
        ...
        <table>
          <thead>
            <tr>
              ...
              <th><i class="fas fa-user-tie"></i> Conducción</th>
            </tr>
          </thead>
          <tbody>
            <?php
            // Consulta SQL dinámica y ordenada cronológicamente por hora de forma automática
            $result = mysqli_query($conn, "SELECT hora, programa, locutor FROM programacion ORDER BY hora ASC");
            
            if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    // Si el campo locutor viene vacío o null desde la BD, asignamos un texto genérico por seguridad
                    $locutor = !empty($row['locutor']) ? htmlspecialchars($row['locutor']) : 'Staff Pakal';
                    
                    echo "<tr>";
                    echo "<td><strong>" . htmlspecialchars($row['hora']) . "</strong></td>";
                    echo "<td>" . htmlspecialchars($row['programa']) . "</td>";
                    echo "<td><span class='badge-staff'><i class='fas fa-headphones-alt me-1'></i> " . $locutor . "</span></td>";
                    echo "</tr>";
                }
            } else {
                echo "<tr><td colspan='3' style='text-align: center; padding: 2.5rem;'>📻 Por el momento no hay horarios registrados en el panel de control.</td></tr>";
            }
            ?>
          </tbody>
        </table>
    ...
    <section id="ubicacion">
      ...
      </table> <!-- Nota: Corregido un pequeño cierre mal hecho en el HTML de tu iframe original -->
      </iframe>
    </section>
  </div>
