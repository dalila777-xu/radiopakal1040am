...
function actualizarCarruselPublico($conn) {
    $items = [];
    ...
    file_put_contents(__DIR__ . DIRECTORY_SEPARATOR . 'carrusel_data.json', json_encode($items, JSON_UNESCAPED_UNICODE));
}
        ...
        if (in_array($tipo_archivo, $formatos_permitidos)) {
            ...
            if (is_uploaded_file($_FILES['imagen']['tmp_name']) && move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta_destino_fs)) {
                ...
                if (mysqli_query($conn, $query)) {
                    actualizarCarruselPublico($conn);
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<div class='card-foto'>";
                   // Añadir un parámetro de versión basado en la fecha de modificación del archivo
                   $ruta_publica = $row['ruta_imagen'];
                   $ruta_fs = __DIR__ . DIRECTORY_SEPARATOR . $ruta_publica;
                   $vers = '';
                   if (file_exists($ruta_fs)) {
                       $vers = '?v=' . filemtime($ruta_fs);
                   }
                   echo "<img src='" . htmlspecialchars($ruta_publica . $vers) . "' alt='Foto Carrusel'>";
