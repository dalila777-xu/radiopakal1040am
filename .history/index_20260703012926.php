          ...
          <div class="carousel">
            <div class="carousel-track-container">
              <ul class="carousel-track">
                <?php 
                  include("admin_carrusel.php");
                  if (!empty($carouselImages) && is_array($carouselImages)) {
                    foreach ($carouselImages as $index => $imagen):
                      $altText = $imagen['alt'] ?? 'Slide ' . ($index + 1);
                      $src = $imagen['src'] ?? 'https://placehold.co/900x400?text=Imagen+' . ($index + 1);
                ?>
                  <li class="carousel-slide"><img src="<?php echo htmlspecialchars($src); ?>" alt="<?php echo htmlspecialchars($altText); ?>" onerror="this.src='https://placehold.co/900x400?text=Radio+Pakal+<?php echo ($index + 1); ?>'"></li>
                <?php 
                    endforeach;
                  } else {
                    // Fallback si no hay imágenes en admin_carrusel.php
                ?>
