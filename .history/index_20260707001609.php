...
// MANEJO DE LOGIN
// ==========================================
// BOTÓN FLOTANTE (3 BARRAS) PARA MOSTRAR/OCULTAR FORMULARIO DE LOGIN
echo '\n<!-- Botón flotante 3 barras -->
<style>
  .float-login-btn{position:fixed;right:16px;bottom:16px;z-index:9999;cursor:pointer}
  .float-login-btn .bar{display:block;width:28px;height:3px;background:#222;margin:5px 0;border-radius:2px}
  .login-modal{position:fixed;right:16px;bottom:64px;z-index:9998;background:#fff;border:1px solid #ccc;padding:12px;border-radius:6px;box-shadow:0 4px 12px rgba(0,0,0,.15);display:none;width:260px}
  .login-modal h4{margin:0 0 8px;font-size:14px}
  .login-modal input[type=text],.login-modal input[type=password]{width:100%;padding:8px;margin:6px 0;border:1px solid #ddd;border-radius:4px}
  .login-modal input[type=submit]{width:100%;padding:8px;margin-top:8px;background:#007bff;color:#fff;border:0;border-radius:4px}
</style>

<div class="float-login-btn" id="floatLoginBtn" title="Acceder como administrador">
  <span class="bar"></span>
  <span class="bar"></span>
  <span class="bar"></span>
</div>

<!-- Modal/box de login: envía a esta misma página para que el manejo existente lo procese -->
<div class="login-modal" id="loginModal">
  <form method="post" action="'.htmlspecialchars($_SERVER['PHP_SELF']).'">
    <h4>Iniciar sesión - Administrador</h4>
    <input type="text" name="username" placeholder="Usuario" required>
    <input type="password" name="password" placeholder="Contraseña" required>
    <input type="hidden" name="admin_login_toggle" value="1">
...
<script>
  (function(){
    var btn = document.getElementById('floatLoginBtn');
