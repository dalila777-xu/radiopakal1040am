        ...
        if (in_array($tipo_archivo, $formatos_permitidos)) {
            ...
            if (move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta_destino)) {
                ...
                if (mysqli_query($conn, $query)) {
                    $mensaje = "<div class='alert success'>📸 Imagen subida correctamente y agregada al carrusel.</div>";
                } else {
...
if (isset($_GET['eliminar'])) {
    ...
    if ($res_buscar && mysqli_num_rows($res_buscar) > 0) {
        ...
        if (mysqli_query($conn, $query_borrar)) {
            $mensaje = "<div class='alert success'>🗑️ Imagen eliminada con éxito del carrusel.</div>";
        } else {
