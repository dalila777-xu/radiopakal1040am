...
$query_publicaciones = "SELECT * FROM publicaciones ORDER BY id DESC";
...
// Obtener imágenes del carrusel (admin_carrusel.php debería llenar la tabla 'carrusel')
$possibleCarouselTables = ['carrusel', 'carousel', 'slider', 'imagenes_carrusel'];
$carrusel_images = [];
foreach ($possibleCarouselTables as $table) {
    $query_carrusel = "SELECT * FROM `" . mysqli_real_escape_string($conn, $table) . "` ORDER BY id DESC";
...
function obtenerImagenCarrusel($item) {
    $campos = ['imagen', 'imagen_url', 'url_imagen', 'img', 'src', 'ruta', 'archivo', 'path', 'url'];
...
function obtenerAltCarrusel($item) {
    $campos = ['alt', 'titulo', 'nombre', 'descripcion', 'texto'];
    foreach ($campos as $campo) {
        if (!empty($item[$campo])) {
            return $item[$campo];
        }
    }
    return 'Imagen del carrusel';
}
          ...
          <div class="carousel">
            <div class="carousel-track-container">
              <ul class="carousel-track">
                <?php if (!empty($carrusel_images)): ?>
                  <?php foreach ($carrusel_images as $item): ?>
                    <?php $imagenCarrusel = obtenerImagenCarrusel($item); ?>
                    <?php if (empty($imagenCarrusel)) continue; ?>
                    <li class="carousel-slide">
                      <img src="<?php echo htmlspecialchars($imagenCarrusel); ?>" alt="<?php echo htmlspecialchars(obtenerAltCarrusel($item)); ?>" onerror="this.onerror=null;this.src='https://placehold.co/900x400?text=Carrusel'">
                    </li>
                  <?php endforeach; ?>
                <?php else: ?>
